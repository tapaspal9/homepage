...
- **Registered FE for attribute-based attribute-weighted sums (AB-AWS)** - the first RFE for AB-AWS with built-in access control realized by ABPs / $\mathsf{L}$ / $\mathsf{NL}$. A function $f=(g,h)$ on input $(\vec y,(\lbrace \vec x_j, \vec z_j\rbrace_{j\in [N]}))$ returns $\sum_{j \in [N]} f(\vec x_j)^{\tr} \vec z_j$ **iff** the policy $g(\vec y)=0$, with public attributes $\vec y, \vec x_j$ and private $\vec z_j$.
